...
namespace ContactPro.Services
{
    public class AddressBookService : IAddressBookService
    {
        ...
        public async Task<bool> IsContactInCategory(int categoryId, int contactId)
        {
            ...
            return await _context.Categories.Include(c => c.Contact)
                                            .Where(c => c.Id == categoryId && c.Contact.Contains(contact))
                                            .AnyAsync();
        }
        ...
        public async Task RemoveContactFromCategoryAsync(int categoryId, int contactId)
        {
            try
            {
                if (await IsContactInCategory(categoryId, contactId))
                {
                    ...
                    {
                        category.Contact.Remove(contact);
                        await _context.SaveChangesAsync();
                    }
